...
| `postgres_store.py` | Same `Observation`/append-only design as `observations.py`, against a live Postgres connection — for higher data volume. Also implements the human-review gate for AI-extracted data (`pending_review` → `approve_run`/`reject_run`), with no external ticketing system required. `Observation`'s `unit`/`row_key`/`dimensions` are optional: `append()` names those columns only when a batch sets them, so a schema without them is unaffected. `dimensions` is an open JSONB dict so a dashboard's own axes need no toolkit change. |
